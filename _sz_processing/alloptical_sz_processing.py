...
for sz_num, stims_of_interest in enumerate(expobj.stimsWithSzWavefront):
    ...
    for stim in stims_of_interest:
        ...
        in_sz, out_sz, fig, ax = expobj.classify_slmtargets_sz_bound(expobj.sz_border_path(stim=stim), stim=stim,
                                                                     to_plot=True, title='%s' % stim, flip=flip,
                                                                     fig=fig, ax=ax)
        expobj.slmtargets_szboundary_stim[
            stim] = in_sz  # for each stim, there will be a ls of cells that will be classified as in seizure or out of seizure
...
assert hasattr(expobj, 'stims_in_sz'), AttributeError(
    f"{expobj.metainfo['animal prep.']} {expobj.metainfo['trial']} doesn't have stims_in_sz attr.")
...
for row in expobj.insz_responses_SLMtargets.index:
    # stims = [expobj.stim_start_frames.index(stim) for stim in expobj.stims_in_sz]
    responses = [expobj.insz_responses_SLMtargets.loc[row][stim] for stim in expobj.stims_in_sz if
                 row not in expobj.slmtargets_szboundary_stim[stim]]
...
data = [list(targets_avgresponses_exclude_stims_sz.values()), list(targets_avgresponses_stims_outsz.values()),
        list(targets_avgresponses_stims_presz.values())]
pj.plot_hist_density(data, x_label='response magnitude (dF/stdF)', title='stims_in_sz - ', figsize=(5, 5),
                     fill_color=['orange', 'skyblue', 'green'],
...
# %%
sz_num = -1
stims_to_plot = [stim for stim in expobj.stim_start_frames if
                 expobj.seizure_lfp_offsets[sz_num] > stim > expobj.seizure_lfp_onsets[sz_num]]
...
for i in range(len(stims_to_plot)):
    ...
    coords_to_plot2 = [expobj.target_coords_all[cell] for cell in
                       expobj.slmtargets_szboundary_stim[stim]]  # targets in sz bound
...
def _trialProcessing_nontargets(expobj, normalize_to='pre-stim', save=True):
    ...
    if hasattr(expobj, 'slmtargets_szboundary_stim'):
        stim_timings_insz = [(x, stim) for x, stim in enumerate(expobj.stim_start_frames) if
...
# %% 5.1) counting seizure INCIDENCE across all imaging trials
...
# 5.1.1) measure seizure incidence across onePstim trials
...
# 5.1.2) plot seizure incidence across onePstim and twoPstim trials
...
# %% 5.2) measure seizure LENGTHS across all imaging trials (including any spont imaging you might have)
...
# 5.2.1) measure seizure incidence across onePstim trials
for pkl_path in onePresults.mean_stim_responses['pkl_list']:
    if list(onePresults.mean_stim_responses.loc[
                onePresults.mean_stim_responses['pkl_list'] == pkl_path, 'post-4ap response (during sz)'])[0] != '-':
...
# 5.2.2) plot seizure length across onePstim and twoPstim trials
...
sznum = 1
slice = np.s_[expobj.convert_frames_to_paqclock(expobj.seizure_lfp_onsets[sznum]): expobj.convert_frames_to_paqclock(
