...
for on, off in zip(on_, end):
    ...
    for stim in stims_of_interest:
        ...
        counter += 1
    fig.suptitle('%s %s - Avg img around stims during sz - seizure # %s' % (expobj.metainfo['animal prep.'], expobj.metainfo['trial'], sz_num + 1), y=0.995)
    fig.show()
    sz_num += 1
...
# %% ##
# all stims in seizures (not broken up by individual sz events - above is copy of same code that is with indiv. sz)
...
for stim in stims_of_interest:
    ...
    axs[counter // ncols, counter % ncols] = ax
    counter += 1
fig.suptitle('%s %s - Avg img around stims during sz - all sz events' % (expobj.metainfo['animal prep.'], expobj.metainfo['trial']), y=0.995)
fig.show()
...
# %% need to repeat the above code
# to correct for mis-assignment of cells (look at results and then find out which stims need to be flipped)
expobj.not_flip_stims = [350, 500, 651,
                         1252, 1402, 1552, 1702, 1852, 2003,
                         2904, 3054, 3205,
                         4106, 4256, 4406, 4557, 4707, 4857
...
# %% responses of targets during seizures, but EXCLUDE STIMS WHERE THE CELL IS INSIDE THE SZ BOUND
