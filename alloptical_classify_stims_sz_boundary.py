...
#  2.2.1) ######## - individual sz events
    ...
    #
            ...
            for stim in stims_of_interest:
                ...
                axs[counter // ncols, counter % ncols] = ax
                counter += 1
            fig.suptitle('%s %s - Avg img around stims during sz # %s' % (
            expobj.metainfo['animal prep.'], expobj.metainfo['trial'], sz_num), y=0.995)
            fig.show()
...
#
# 2.2.2) ####### all stims in seizures (not broken up by individual sz events - above is copy of same code that is with indiv. sz)
#         stims_of_interest = [stim for stim in expobj.stims_in_sz[1:]]
...
#
#             # classification of suite2p ROIs relative to sz boundary
#             in_sz, out_sz, fig, ax = expobj.classify_cells_sz_bound(sz_border_path, stim=stim, to_plot=True, title='%s' % stim,
#                                                                     flip=flip, fig=fig, ax=ax, text='sz %s stim %s' % ())
#             # classification of SLM targets relative to sz boundary
#             # in_sz, out_sz, fig, ax = expobj.classify_slmtargets_sz_bound(sz_border_path, stim=stim, to_plot=True, title='%s' % stim, flip=flip, fig=fig, ax=ax)
