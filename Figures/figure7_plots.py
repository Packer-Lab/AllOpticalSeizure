...
def z_score_response_proximal_distal(results=results, **kwargs):
    ...
    fig, ax = pplot.plot_bar_with_points(data=[ pj.flattenOnce(interictal_responses), pj.flattenOnce(proximal_responses), pj.flattenOnce(distal_responses)], points=False,
                               paired=False, bar=True, colors=['forestgreen', '#db5aac', '#dbd25a'], edgecolor='black', lw = 0.8, capsize=3.5,
