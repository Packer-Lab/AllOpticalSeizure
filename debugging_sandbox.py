...
#
# sig_responders_avgresponse = np.mean(expobj.post_array_responses[expobj.sig_units], axis=1)
# nonsig_responders_avgresponse = np.mean(expobj.post_array_responses[~expobj.sig_units], axis=1)
# data = np.asarray([sig_responders_avgresponse, nonsig_responders_avgresponse])

data = np.random.random((2, 200)) - 0.5
plot_bar_with_points(data, title='Avg stim response magnitude of cells', colors=['green', 'gray'], y_label='avg dF/stdF', bar=False, text_list=['n = 97.%', 'n = 0.5%'],
                        x_tick_labels=['significant', 'non-significant'], expand_size_y=1.5, expand_size_x=0.8)