...
class TargetsSzInvasionTemporal(Quantification):
    ...
    # %% 5) COLLECT PHOTOSTIM RESPONSES MAGNITUDE (ZSCORED) PRE AND POST SEIZURE INVASION FOR EACH TARGET ACROSS ALL EXPERIMENTS  ########################################################
    def collect_szinvasiontime_vs_photostimresponses_zscored(self, expobj: Post4ap, zscore_type: str = 'dFF (zscored)'):
        ...
        for idx, stim in enumerate(stims_list):
            ...
            sztime_v_photostimresponses_zscored[stim]['time_to_szinvasion'] += list(stim_timesz_df[stim][stim_timesz_df[stim].notnull()])
            sztime_v_photostimresponses_zscored[stim]['photostim_responses_zscored'] += list(expobj.PhotostimResponsesSLMTargets.adata.layers[zscore_type][:, idx][stim_timesz_df[stim].notnull()])
            ...
            # print(len(np.unique(sztime_v_photostimresponses[stim]['time_to_szinvasion'])))
            # print(len(np.unique(sztime_v_photostimresponses[stim]['photostim_responses'])))
            assert len(sztime_v_photostimresponses_zscored[stim]['time_to_szinvasion']) == len(sztime_v_photostimresponses_zscored[stim]['photostim_responses_zscored'])
            assert sum(np.isnan(sztime_v_photostimresponses_zscored[stim]['time_to_szinvasion'])) == 0, print('something went wrong - found an nan in time to szinvasion')
