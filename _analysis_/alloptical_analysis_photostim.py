...
expobj = aoutils.import_expobj(prep='RL109', trial='t-013')
...
allopticalResults.post_4ap_trials = [
    ...
    ['RL109 t-021'],
    ['RL109 t-018'],
    ['RL109 t-016'],  #'RL109 t-017'], # -- missing seizure locations for first seizure (need to mark seizure stims)
...
for exptype in ['post', 'pre']:
    for key in allopticalResults.trial_maps[exptype].keys():
        ...
        # aoutils.s2pMaskStack(obj=expobj, pkl_list=[expobj.save_path], s2p_path=expobj.s2p_path, parent_folder=expobj.analysis_save_path, force_redo=True)
        #
