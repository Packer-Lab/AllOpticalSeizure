"""
all  optical interrogation in focal onset model of epilepsy
"""
figname = 'aim3_2p-stim-interictal-01'
...
layout = {
    ...
    'bottom-C': {'panel_shape': (2, 1),
                 'bound': (0.25, 0.47, 0.56, 0.60),
                 'wspace': 0.6},
    'bottom-D': {'panel_shape': (1, 1),
                 'bound': (0.66, 0.47, 0.75, 0.60),
                 'wspace': 0.4},
...
bbox = Bbox.from_extents(0.02, 0.445, 0.16, 0.595)
...
ax.set_yticklabels(['0.1', '0.2', '0.3'], fontsize=8)  # Less radial ticks
...
# %% E - splitting responses during interictal phases - preictal vs. postictal vs. all
...
# interictal_responses = [np.mean(responses) for responses in results.interictal_responses['very_interictal_responses']]
# preictal_responses = [np.mean(responses) for responses in results.interictal_responses['preictal_responses']]
# postictal_responses = [np.mean(responses) for responses in results.interictal_responses['postictal_responses']]

to_plot = [interictal_responses, preictal_responses, postictal_responses]

plot_bar_with_points(data=to_plot, bar=True, fontsize=10, points_lw=0.5, points=False,
                     x_tick_labels=['All', 'Pre-ictal', 'Post-ictal'], colors=['gold', 'lightseagreen', 'lightcoral'],
                     y_label='Photostim. response', ylims=[-0.15, 0.45], title='',
                     alpha=1,s=15, capsize=2,
                    ax=ax, show=False,
                    )
...
# %% D) GRAND AVERAGE PHOTOSTIM TRACES AND AVERAGE ACROSS EXPERIMENTS
...
# %% F) BAR PLOT OF AVG PHOTOSTIMULATION FOV RAW FLU ACROSS CONDITIONS
