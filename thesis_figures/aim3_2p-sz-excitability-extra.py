...
layout = {
    'left': {'panel_shape': (1, 1),
                      'bound': (0.07, 0.77, 0.42, 0.92)},
    'right': {'panel_shape': (1, 1),
                   'bound': (0.52, 0.77, 0.87, 0.92),
                   'wspace': 0.2}
}
...
main_spatial.plot__responses_v_distance_no_normalization_rolling_bins(results=results_spatial, fig=fig, axes=[ax])
ax.axhline(0, ls='--', lw=1, color='black', zorder=0)
# ax.set_ylabel('dF/prestimF\n(norm. to baseline)')
# ax.set_ylabel('dF/stdF\n(norm. to baseline)')
# ax.set_ylabel('dFF\n(norm. to baseline)')
# ax.set_title(f'SLM Targets in penumbra')
ax.set_ylabel(r'Responses (dF/F)')
ax.set_xlabel(r'Distance to seizure wavefront ($\mu$$\it{m}$)')

rfv.add_label_axes(text='A', ax=ax, x_adjust=0.05)
...
avg_responses = results_spatial.rolling_binned__distance_vs_photostimresponses['avg_photostim_response_in_bin'][:-bin_width]
...
def regression_fit(x, y, **kwargs):
    ...
    ## TEST PLOT OF y_pred
    if 'ax' in kwargs:
        kwargs['ax'].plot(x, y_pred, lw=2, color='blue', ls='--', label='3rd-poly')
...
regression_fit(distances, avg_responses, ax=axes['right'][0])

# fig.show()
ax = axes['right'][0]
# ax.set_title(f'Photostimulation targets in penumbra')
ax.set_ylabel('Responses (z-score)')
ax.set_xlim([0, 450])
ax.set_xlabel(r'Distance to seizure wavefront ($\mu$$\it{m}$)')

rfv.add_label_axes(text='B', ax=ax, x_adjust=0.05)
