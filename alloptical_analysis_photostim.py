...
######### ZONE FOR CALLING THIS SCRIPT DIRECTLY FROM THE SSH SERVER ###########
"""
# %% 6.2) DATA COLLECTION - absolute stim responses vs. TIME to seizure onset - responses: delta(dFF) from whole trace - for loop over all experiments to collect responses in terms of sz onset time
...
# %% 6.1-dc) DATA COLLECTION - absolute stim responses vs. TIME to seizure onset - responses: dF/prestimF - for loop over all experiments to collect responses in terms of sz onset time
...
# %% 6.1-dc) DATA COLLECTION: organize SLMTargets stim responses - across all appropriate pre4ap, post4ap trial comparisons - using whole trace dFF responses - TODO need to modify to use new whole trace dFF responses
...
for i in range(len(allopticalResults.pre_4ap_trials)):
    ...
    if f"{prep} {pre4aptrial}" not in trials_skip:
        ...
        stim_responses_tracedFF_comparisons_dict[prep][f'{comparison_number}'] = {'pre-4ap': {}}
...
# %% 6) DATA COLLECTION: organize SLMTargets stim responses - across all appropriate pre4ap, post4ap trial comparisons - responses are dF/prestimF
""" doing it in this way so that its easy to use in the response vs. stim times relative to seizure onset code (as this has already been coded up)"""
